...
use Jurager\Passport\PassportGuard;
use Jurager\Passport\ClientBrokerManager;
use Jurager\Passport\Events;

class PassportGuardTest extends TestCase
{
    protected $provider;
    ...
    public function setUp()
    {
        ...
        $this->guard = new PassportGuard($this->provider, $this->broker);
    ...
    {
        $this->app['config']->set('passport.broker_client_username', 'username');
        $user = (object) ['id' => 1];
    ...
    {
        $user = new class {
            use \Jurager\Passport\Traits\PassportUser;
            public $id = 1;
        };
    ...
    public function testShouldAttemptToConnectAndSucceedWithUserStrategy()
    {
        ...
        $this->app['config']->set('passport.user_create_strategy', function($data) use ($user) {
            return $user;
        });
